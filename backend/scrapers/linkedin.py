...
TECH_KEYWORDS = [
    ...
    "angular", "java", "spring", "rails", "ruby", "express", "django",
]


class LinkedInScraper(BaseScraper):
    """Scrapes LinkedIn jobs via python-jobspy. Filters out Easy Apply."""
    ...
    def scrape(self) -> List[RawJob]:
        ...
        seen_ids: set = set()
        search_terms = self.config.sources.linkedin.search_terms
        ...
        for term in search_terms:
            try:
                ...
                df = scrape_jobs(
                    ...
                    search_term=term,
                    location="India",
                    results_wanted=self.config.sources.linkedin.max_results,
                ...
                for _, row in df.iterrows():
                    ...
                    # CRITICAL: Filter out Easy Apply jobs immediately
                    if "linkedin.com/jobs/apply" in apply_url:
                        continue

                    # Deduplicate across search terms
                    job_id_raw = f"{str(row.get('company', '')).lower()}{str(row.get('title', '')).lower()}linkedin"
